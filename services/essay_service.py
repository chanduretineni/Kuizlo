...
async def generate_body_paragraph(topic: str, references: list[ReferenceObject], target_words: int, target_total_words: int, generated_words: int) -> str:
    try:
        generated_word_count = 0
        body_paragraph = ""
        while generated_word_count < target_words:
...
async def generate_essay_logic(request: GenerateEssayRequest):
    try:
        ...
        # Generate the body
        body = await generate_body_paragraph(request.topic, request.selected_references, body_words, request.wordCount, generated_words)
        ...
        # Combine all sections into the final essay text
        essay_text = f"{introduction}\n\n{body}\n\n{conclusion}"

        # Save the generated essay to a file
        ...
        logging.info(f"Essay generation logic executed in {execution_time:.2f} seconds for {len(essay_text.split())} words.")
